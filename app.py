...
def messages(username=None):
    contacts = database.get_contacts(mysql, g.user['user_id'])
    if username == None and len(contacts) > 0:
        ...
        return render_template('index.html', 
            ...
            messages='',
            files_shared='')
    active_contact = database.get_user(mysql, username)
    messages = database.get_messages(mysql, g.user['user_id'], username)
    files = database.get_files(mysql, g.user['user_id'], username)
...
def server_login():
    ...
    result = database.login(mysql, username, password)
    if len(result) > 0:
...
def server_search():
    ...
    result = jsonify(database.search(mysql, g.user['user_id'], user_query))
    return result

@app.route('/server/add_contact', methods=['POST'])
def server_add_contact():
    username = request.form['username']
    database.add_contact(mysql, g.user['user_id'], username)
    return "0"
...
def save_file(sent_file, sender, recipient):
    ...
    if(sent_file != ''):
        ...
        database.save_file(mysql, new_filename, sender, recipient)
        destination = '{}{}{}'.format(
            target,
