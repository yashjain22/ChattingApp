class DatabaseManager:
    ...
    def login(self,mysql, username, password):
    ...
    def search(self,mysql, user_id, user_query):
        cur = mysql.connection.cursor()
        ...
        for result in results:
            ...
            if(self.is_friends(mysql, user_id, result['username'])):
                result['is_friends'] = True
        cur.close()
        print(results)
        return results

    def get_user_id(self,mysql,username):
        cur = mysql.connection.cursor()
    ...
    def add_contact(self,mysql, user_id, username):
        cur = mysql.connection.cursor()
        other_contact = self.get_user_id(mysql,username)
        sql_query = 'INSERT INTO friends_list SET user_1 = %s, user_2 = %s'
        cur.execute(sql_query, [user_id, other_contact])
        mysql.connection.commit()

    def is_friends(self,mysql, user_id, username):
        cur = mysql.connection.cursor()
        other_contact = self.get_user_id(mysql, username)
    ...
    def get_contacts(self,mysql, user_id):
        cur = mysql.connection.cursor()
    ...
    def get_user(self,mysql, username):
        cur = mysql.connection.cursor()
    ...
    def get_last_message(self,mysql, user_id, username):
        cur = mysql.connection.cursor()
        sql_query = '''SELECT message FROM messages WHERE'''

    def send_message(self,mysql, message, sender, recipient):
        cur = mysql.connection.cursor()
        other_contact = self.get_user_id(mysql, recipient)
        user_id = self.get_user_id(mysql, sender)
    ...
    def get_messages(self,mysql, user_id, username):
        cur = mysql.connection.cursor()
        other_contact = self.get_user_id(mysql, username)
    ...
    def save_file(self,mysql, file_info, sender, recipient):
        cur = mysql.connection.cursor()
        other_contact = self.get_user_id(mysql, recipient)
    ...
    def get_files(self,mysql, user_id, username):
        cur = mysql.connection.cursor()
        other_contact = self.get_user_id(mysql, username)
    ...
    def get_file(self,mysql, file_id):
